...
@app.get('/')
def message():
    return 'PROYECTO INTEGRADOR LUCAS SALZOTTO'
...
def PlayTimeGenre(genre: str) -> dict:
    ...
    return {"Genre": genre, "Year of release with the most playtime for Genre: ": int(max_playtime_year)}
...
def UserForGenre(genre: str) -> dict:
    ...
    result = {
        "User with the most playtime for Genre " + genre: max_playtime_user,
        "Played Hours": playtime_list}
    return result
...
def recomendacion(id_producto: int):
    if id_producto not in muestra['steam_id'].values:
        return {'mensaje': 'The game ID does not exist.'}
    ...
    return {'Recommended Games': list(sim_juegos)}
